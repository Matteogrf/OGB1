...
class Message(object):
    def __init__(self, id, player, coords, loot, fleet, defense):
        self.id = id
        self.player = player
        self.loot = loot
        self.coords = coords
    ...
    def is_in_range(self, coords, radius):
        gPianeta, sPianeta, pPianeta = map(int, self.coords.split(":"))
        gTarget, sTarget, pTarget = map(int, coords.split(":"))

        # Controllo galassia
        if gPianeta <> gTarget:
            return False

        sMax = (sPianeta + radius) % 500
        sMin = (500 + sPianeta - radius) % 500

        if (sMin < sMax) and (sTarget < sMin or sTarget > sMax):
            return False

        if (sMin > sMax) and sTarget > sMax and sTarget < sMin:
            return False
